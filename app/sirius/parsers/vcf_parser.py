...
class VCFParser_ExAC(VCFParser):
    def get_mongo_nodes(self):
        ...
        >>> parser = VCFParser_ExAC('test.vcf')
        >>> parser.parse()
...
class VCFParser_VEP(VCFParser):
    def get_mongo_nodes(self):
        """
        Parser specifically designed to handle VCF files generated by ENSEMBL VEP pipeline.
        Parse self.data into three types for Mongo nodes, which are the internal data structure in our MongoDB.
        ...
        which are contents of self.data
        2. The GenomeNodes generated from this parsing should be variants. They should all have "_id" started with "G". The rs# will be used, like "Gsnp_rs4950928".
        3. The info fields are named consistently with the VCFParser_Exac
        4. The CSQ sub-documents have all or part of these labels:
        CSQ_LABELS = "Allele|Consequence|IMPACT|SYMBOL|Gene|Feature_type|Feature|BIOTYPE|EXON|INTRON|HGVSc|HGVSp|cDNA_position|CDS_position|
