...
def get_aggregation_segments(gnome_in_range, annotation, sampling_rate, track_height_px):
    """ Optimized clustering algorithm that handles arbitrary size of data, with a certain resolution """
    ndata = len(gnome_in_range)
    if ndata == 0: return []
    elif ndata == 1:
        cluster_ends = [ndata]
    else:
        ...
        for gnome_data in gnome_in_range:
            abs_start, abs_end, _, _ = gnome_data
            pos.append(0.5 * (abs_start + abs_end))
    ...
    for c_end in cluster_ends:
        r_cluster = gnome_in_range[c_begin: c_end+1]
        c_size = len(r_cluster)
        label = str(c_size)
        startBp = r_cluster[0][0]
        endBp = r_cluster[-1][1]
